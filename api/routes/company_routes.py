...
def get_company(company_id):
    ...
    sig_row = db.session.execute(
        db.text("""
            SELECT digital_initiatives, leadership_changes, hiring_signals,
                   ...
                   digital_maturity_score, fiscal_year_end,
                   it_spend_indicators, tech_stack_categories,
                   enriched_at, enrichment_cost_usd
    ...
    if sig_row:
        l2_modules["signals"] = {
            ...
            "regulatory_pressure": sig_row[11],
            "employee_sentiment": sig_row[12],
            "digital_maturity_score": sig_row[13],
            "fiscal_year_end": sig_row[14],
            "it_spend_indicators": sig_row[15],
            "tech_stack_categories": sig_row[16],
            "enriched_at": _iso(sig_row[17]),
            "enrichment_cost_usd": float(sig_row[18])
            if sig_row[18] is not None
            else None,
        }
    ...
    opp_row = db.session.execute(
        db.text("""
            SELECT pain_hypothesis, relevant_case_study, ai_opportunities,
                   ...
                   adoption_barriers, pitch_framing,
                   enriched_at, enrichment_cost_usd
    ...
    if opp_row:
        l2_modules["opportunity"] = {
            ...
            "enriched_at": _iso(opp_row[8]),
            "enrichment_cost_usd": float(opp_row[9])
            if opp_row[9] is not None
            else None,
        }
