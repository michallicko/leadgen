...
specialist subgraph (Strategy, Research) or handles quick answers
directly. It manages context distribution and result synthesis.
...
Flow:
  User message -> classify_intent -> route_to_agent
      ...
      +------------+-------------+------+----------+
      |            |             |                  |
  strategy    research    quick_response    passthrough
      |            |             |                  |
      +------------+-------------+------+----------+
                                        |
                                    synthesize -> END
...
def passthrough_node(state: AgentState) -> dict:
    ...
    monolithic graph behavior. This node is a placeholder for
    future Outreach/Campaign Agent subgraphs.
    """
    # For now, route campaign intents to quick_response
    # In future sprints, this will route to a Campaign Agent subgraph
...
def route_to_agent(
    ...
    "strategy_node", "research_node", "quick_response_node", "passthrough_node"
]:
